...
from benchmarks import whoIsMaster
        ...
        async with ClientSession() as session:
            async with session.post(url, data={'txid': 'new' * 1000}) as resp:
                ...
                print(count, response,
                      str(datetime.now()))  # strftime("%Y-%m-%d %H:%M:%S", gmtime()) str(datetime.now()) time.ctime()
...
#url2 = 'http://127.0.0.1:8000/tx'
#url2 = 'http://127.0.0.1:8000/redirect2Y'
url2 = 'http://10.20.42.88:8000/tx'

loop = asyncio.get_event_loop()
# loop.run_until_complete(get(url, secs))
loop.run_until_complete(post(url2, secs))
#loop.run_until_complete(post(url, secs))
