...
class Crypto():
    ...
    def verifyMsgSig(self, signed_msg, verifying_key, print_verified=True):
        ...
        try:
            ...
            print('Exception for %s, ErrorLine: %s' % (signed_msg, ex.__traceback__.tb_lineno)) #TODO log?
            return False, None
        ...
        while True:
            try:
                ...
                if not self.TASKS.verify_processing and now - self.TASKS.start_time >= self.TASKS.RUN_SECS: #TODO tools.config.TASK_VERIFY_SDB_INTERVAL_SECS
                    ...
                    #print("%s Started Task verify_processing" % tools.utc())
                    #time.sleep(1)
                    verify_q = tools.SERVICE_DB.queryServiceDB("select * from v1_pending_msg as p where p.signed_msg_hash not in (select signed_msg_hash from v1_verified_msg) order by msg_priority desc, node_date asc") #todo limitBYconfig where node_verified='0'
                        ...
                        if tools.Node.isNodeValid("W" + tools.to_HMAC(pubk)):
                            ...
                            #continue
                                ...
                                for i in range(ptxs_count):
                                    msg_sender_wallet = "W" + tools.to_HMAC(verified_msgs[i][3]) #.encode()
                                    ...
                                    marked_spent_itxs, updated_wallet, db_updates = tools.markSpentTxRecordsInTheWallet(msg_inputs_ids, block_persist_msgs[msg_sender_wallet], "B" + signed_msg_hash)
                                    ...
                                    for wallet in ctx_receivers_wallets:
                                        if not wallet in block_persist_msgs: #block_recievers_wallets.keys():
                                            ...
                                            block_persist_msgs[wallet] = tools.getDbWalletDefault(wallet)
                                    ...
                                    for m in range(len(msg_ctxs)):
                                        ...
                                        ctx_reciever = msg_ctxs[m][3].decode() if isinstance(msg_ctxs[m][3], bytes) else msg_ctxs[m][3]
                                        ...
                                        # block_recievers_wallets[ctx_reciever]["assets"][ctx_asset]["inputs"].append(
                                ...
                                try:
                                    for msg in verified_msgs: #todo redo validation hash(msg,pubk)
                                        umsg = unpackb(msg[2])
                                        #msg_sender_wallet = b"W" + tools.to_HMAC(umsg[-1]).encode()
                                        # msg_sender_wallet_data = tools.getDbWalletDefault(msg_sender_wallet)
                                        ...
                                        for wallet in msg_receivers_wallets:
                                            if not wallet in block_persist_msgs.keys():
                                                ...
                                                block_recievers_wallets.append(wallet) #[wallet] = msg_receiver_wallet_data
                                    ...
                                    print("#####TODO -outer in start BlockCalcPtxHash #1", tools.to_HMAC( packb((verified_msgs[0][1], (verified_msgs[0][3])))))
                                    ...
                                    tools.SERVICE_DB.deleteSdbPendingMsgsIfVerified()
                                    tools.SERVICE_DB.deleteBlockSdbVerifiedMsgs(block_persist_msgs.keys())
                                    tools.SERVICE_DB.deleteBlockSdbVerifiedMsgs([signed_msg_hash]) #del verified block
                                except Exception as ex:
                                    tools.SERVICE_DB.deleteSdbPendingMsgsIfVerified()
                            ...
                            # TODO compute/calc MsgHash
                                ...
                                if not self.TASKS.isNone(isVerified):
                                    ...
                                    #assert signed_msg_hash == tools.to_HMAC(umsg[1])
                                    print("Processing msg: %s\n%s" % (signed_msg_hash, umsg))
                                    umsg[2] = [unpackb(msg) for msg in umsg[2]]
                    ...
                    print("sdb verified count: ", tools.SERVICE_DB.queryServiceDB("select count(*) from v1_verified_msg"))
...
##        assert res
        ...
        # if ret is None:
        #     tools.Node.TASKS.deleteSdbVerifiedMsqQ.add(smsg[3])
        return ret
...
#            in_db = tools.getDbRec(msg, tools.DB.DB_PATH)
   ...
   #TODO read from protected pwd binary encoded + verifyExistingBeforeUPdateWallet
    ...
    ptx = tools.WALLET.createPtx(gVK2._key, [tools.config.MAIN_COIN, tools.config.MAIN_COIN, tools.config.MAIN_COIN], [b'1', b'1', b'1'], to_addrs)

    assert not ptx is None
    print("WALLET.createPtx 3 payments - valid PTX : ", ptx)
    smsg = tools.WALLET.signMsg(ptx, gSK2, gVK2._key)
    print("VALID PTX HMAC: ", smsg[3])
    umsg = unpackb(smsg[0])
    vmsg = umsg[1][0]
    ...
    ptx = tools.WALLET.createPtx(gVK2._key, [tools.config.MAIN_COIN], [b'1'], [to_addrs[0]])
    isOk = tools.signAndSendPtx(gSK2, gVK2._key, ptx)  # #tools.sendMsgZmqReq(smsg[0], 'localhost', tools.Node.PORT_REP)
    ...
    print("tools.Block.getLastBlockId()", tools.Block.getLastBlockId())
    # print("*****1st BLOCK validMsg - with INVALID TX inside*****")
    block_msg = (tools.MsgType.Type.VERSION.value, tools.MsgType.Type.BLOCK_MSG.value,
                 '1', tools.Block.getLastBlockId().encode(),
                 msg_list, [b"ToDo_VerifyMinerSigs_turns_and_amounts"], tools.utc_timestamp_b())
    ...
    sdb_tx = tuple(list_stx[:tx_fields_len])
    db_tx = btx
    db_tx += (unpackb(wmsg)[1],)
    db_tx += (unpackb(wmsg)[2],)
    assert sdb_tx == db_tx
    assert packb((packb(sdb_tx[:-2]), sdb_tx[-2], sdb_tx[-1])) == wmsg
